...
class AlbumRecyclerViewAdapter(val context: Context, var data: ArrayList<AlbumFile>) :
        RecyclerView.Adapter<AlbumRecyclerViewAdapter.ViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(context)
                .inflate(R.layout.adapter_pet_image, parent, false)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val albumFile = data[position]
        holder.myImageView.setImageBase64(encodeToBase64(BitmapFactory.decodeFile(albumFile.path),
